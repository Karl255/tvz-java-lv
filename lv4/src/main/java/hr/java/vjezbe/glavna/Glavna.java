...
public class Glavna {
	...
	public static void main(String[] args) {
		...
		for (var ustanova : obrazovneUstanove) {
			...
			for (var p : ustanova.getPredmeti()) {
				ispisiStudenteNaPredmetu(scanner, p);
			}
	...
	private static void ispisiStudenteNaPredmetu(Scanner scanner, Predmet p) {
		System.out.print("Odaberite smjer sortiranja (-1 ili 1): ");
		int sortOrder = SafeScanning.ensureNextInt(scanner, n -> n == 1 || n == -1);
		scanner.nextLine();
		
		System.out.printf("Studenti upisani na predmet %s (%s) (prema dobi):%n", p.getNaziv(), p.getSifra());
		...
		*/
		var doba = p.getStudentiPremaDobi().entrySet().toArray();
		
		for (int i = 0; i < doba.length - 1; i++) {
			for (int j = i + 1; j < doba.length; j++) {
				if (((Map.Entry<Integer, List<Student>>)doba[i]).getKey().compareTo(((Map.Entry<Integer, List<Student>>)doba[j]).getKey()) != sortOrder) {
					var t = doba[i];
					doba[i] = doba[j];
					doba[j] = t;
				}
		...
		for (var dob : doba) {
			var dobT = (Map.Entry<Integer, List<Student>>)dob;
			for (var s : dobT.getValue()) {
				System.out.printf("%s %s %s (%d)%n", s.getJmbag(), s.getPrezime(), s.getIme(), dobT.getKey());
			}
		}
	}
	 ...
	 */
	static Student odaberiStudenta(Scanner scanner, List<Student> studenti) {
		for (int i = 0; i < studenti.size(); i++) {
			System.out.printf("  %d. %s %s%n", i + 1, studenti.get(i).getIme(), studenti.get(i).getPrezime());
		}
	 ...
	 */
	static List<Student> odaberiUpisaneStudente(Scanner scanner, List<Student> sviStudenti) {
		...
		List<Student> upisaniStudenti = new ArrayList<>();
		
		for (int i = 0; i < indecies.length; i++) {
			if (!upisaniStudenti.contains(sviStudenti.get(i))) {
				upisaniStudenti.add(sviStudenti.get(i));
			} else {
				System.out.println();
			}
		}
