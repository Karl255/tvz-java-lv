...
import hr.java.vjezbe.podaci.IspitDatotekaDatasource;
...
public class GlavnaDatoteke {
	...
	public static void main(String[] args) {
		...
		var ispitSource = new IspitDatotekaDatasource(Path.of(ISPITI_FILE), predmeti, studenti);
		var ispiti = ispitSource.dohvatiPodatke();
		...
		var byGodina = ispiti.values().stream()
			.collect(Collectors.groupingBy(i -> i.getDatumIVrijeme().getYear()));
		
		for (var group : byGodina.entrySet()) {
			int godina = group.getKey();
			Path godinaDir = Path.of("dat", "labos", String.format("Ispiti_%d", godina));
			try {
				Files.createDirectory(godinaDir);
			} catch (FileAlreadyExistsException e) {
			} catch (IOException e) {
				throw new RuntimeException(e);
			}

			for (var g2 : group.getValue().stream().collect(Collectors.groupingBy(i -> i.getPredmet().getNaziv())).entrySet()) {
				String predmet = g2.getKey();
				Path predmetDir = Path.of(godinaDir.toString(), predmet);
				try {
					Files.createDirectory(predmetDir);
				} catch (FileAlreadyExistsException e) {
				} catch (IOException e) {
					throw new RuntimeException(e);
				}
				
				for (var ispitGrupa : g2.getValue().stream().collect(Collectors.groupingBy(i -> i.getDatumIVrijeme().toLocalTime())).entrySet()) {
					Ispit primjer = ispitGrupa.getValue().get(0);
					String destinationFile = String.format("ispit_%02d_%02d.dat", primjer.getDatumIVrijeme().getHour(), primjer.getDatumIVrijeme().getMinute());
					var noviIspitiSource = new IspitDatotekaDatasource(Path.of(predmetDir.toString(), destinationFile), predmeti, studenti);
					
					noviIspitiSource.spremiPodatke(ispitGrupa.getValue());
				}
			}
		}
	}
