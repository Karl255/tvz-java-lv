...
public class Glavna {
	...
	public static void main(String[] args) {
		...
		System.out.print("Unesite ime sveučilišta: ");
		String imeSveucilista = scanner.nextLine();
		...
		Sveuciliste<ObrazovnaUstanova, String> sveuciliste = new Sveuciliste<>(imeSveucilista);
	...
	private static void ispisiStudenteNaPredmetu(Scanner scanner, Predmet p) {
		System.out.print("Odaberite prema kojoj generaciji želite filtrirati (X, Y, Z, SVE): ");
		...
		p.getStudenti().stream()
			.filter(odabir.equals("SVE")
				? x -> true
				: x -> Generacija.parse(odabir).isWithin(x.getDatumRodjenja()))
			.toList()
			.forEach(s -> System.out.printf("  %s %s %s (%s)%n", s.getJmbag(), s.getPrezime(), s.getIme(), s.getDatumRodjenja().format(DateTimeFormatter.ofPattern("d.M.yyyy."))));
	}
	...
	static Student nextStudent(Scanner scanner) {
		...
		System.out.println("  tip studenta (0 = izvanredan, 1 = redovan)");
		var tipStudenta = TipStudenta.parse(SafeScanning.ensureNextInt(scanner, i -> i >= 0 && i <= 1));
		
		return new Student(ime, prezime, jmbag, datumRodjenja, tipStudenta);
	}
