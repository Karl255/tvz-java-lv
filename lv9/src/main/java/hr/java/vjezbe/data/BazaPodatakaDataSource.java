...
public class BazaPodatakaDataSource implements DataSource, Closeable {
	...
	private void deleteGeneric(String query, long id) throws DataSourceException {
		try (var s = connection.prepareStatement(query)) {
			s.setLong(1, id);
			s.execute();
		} catch (SQLException e) {
	...
	@Override
	public void deleteProfesor(long id) throws DataSourceException {
		deleteGeneric("DELETE FROM profesor WHERE id = ?;", id);
	}
	
	@Override
	public void updateProfesor(Profesor profesor) throws DataSourceException {
		try (var s = connection.prepareStatement("UPDATE profesor SET ime = ?, prezime = ?, sifra = ?, titula = ? WHERE id = ?;")) {
			s.setString(1, profesor.getIme());
			s.setString(2, profesor.getPrezime());
			s.setString(3, profesor.getSifra());
			s.setString(4, profesor.getTitula());
			s.setLong(5, profesor.getId());
			s.execute();
		} catch (SQLException e) {
			throw new DataSourceException(e);
		}
